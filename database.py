        ...
        if USE_POSTGRES:
            _exec(cur, 
                """INSERT INTO future_trades
                   ...
                   RETURNING id""",
                (account, pair.upper(), side.upper(), lot, n_positions,
                 exec_h, exec_m, exec_s, close_h, close_m, close_s,
                 now, fire))
...
def log_fired(schedule_id: int, account: int, pair: str, side: str,
              lot: float, kind: str, ticket: str, ok: bool, detail: str) -> None:
    import datetime as dt
    with _conn() as c:
        cur = c.cursor()
        if USE_POSTGRES:
            _exec(cur,
                """INSERT INTO fired (schedule_id, account, pair, side, lot, kind,
                   ticket, ok, detail, at) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)""",
                (schedule_id, account, pair, side, lot, kind, ticket,
                 ok, detail, dt.datetime.now(dt.timezone.utc)))
        else:
            _exec(cur,
                """INSERT INTO fired (schedule_id, account, pair, side, lot, kind,
                   ticket, ok, detail, at) VALUES (?,?,?,?,?,?,?,?,?,?)""",
                (schedule_id, account, pair, side, lot, kind, ticket,
                 1 if ok else 0, detail, dt.datetime.now(dt.timezone.utc).isoformat(timespec="seconds")))
