...
def test_register_close_tomorrow_when_past():
    ...
    when, pair = sch._closes["1:EURUSD"]      # value = (when, pair)
    now = dt.datetime.now(dt.timezone.utc)
...
def test_close_one_retries_then_succeeds(monkeypatch):
    """A transiently failing close is retried after 500 ms (max 3)."""
    import executor as ex
    calls = {"n": 0}

    class FakeCmd:
        def close_all(self, pair=None):
            calls["n"] += 1
            if calls["n"] < 3:
                return False, "requote"
            return True, "closed 2 positions"

    monkeypatch.setattr(ex, "sender_for", lambda acc: FakeCmd())
    monkeypatch.setattr(ex.time, "sleep", lambda s: None)
    ok, detail = ex._close_one(1, "EURUSD")
    assert ok and calls["n"] == 3


def test_close_one_gives_up_after_max(monkeypatch):
    import executor as ex
    calls = {"n": 0}
    ...
    class FakeCmd:
        def close_all(self, pair=None):
            calls["n"] += 1
            return False, "price off"

    monkeypatch.setattr(ex, "sender_for", lambda acc: FakeCmd())
    monkeypatch.setattr(ex.time, "sleep", lambda s: None)
    ok, detail = ex._close_one(2, "XAUUSD247")
    assert not ok and calls["n"] == ex.RETRY_MAX
