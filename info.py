...
terminals (accounts read from acc.env, data from the SpotDump EA bridge).
...
count and live SPREADS (pts) for every symbol the terminal quotes.
...
def render_account(inst: int, head: dict, spots: dict, expected: str) -> str:
    ...
    out.append(f"  {DIM}{'SYMBOL':<8}{'BID':>11}{'ASK':>11}{'SPREAD':>10}{RESET}")
    for sym in quoted:
        bid, ask, _ = spots.get(sym, ("", "", ""))
        ...
        try:
            ...
            b = a = "-"
        out.append(f"  {sym:<8}{b:>11}{a:>11}  {spread(sym, spots)}")
    return "\n".join(out)
...
def build_frame() -> str:
    accs = read_accounts()
    spots = read_spots()
    out = [f"{BOLD}MT5 ACCOUNT INFO{RESET}  {DIM}two terminals via SpotDump "
           f"EA bridge  -  {dt.datetime.now():%Y-%m-%d %H:%M:%S}{RESET}", ""]
