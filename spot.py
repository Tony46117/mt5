...
ACCOUNTS
    acc.env holds login / password / server for account1 and account2.
    Each terminal instance is logged into one account, so monitor.py
    reads the terminal whose trades.csv header reports account1's login,
    and the one reporting account2's login.
...
TERMINALS = {
    1: {"dir": MT5_DIR, "ini": WINEPREFIX / "drive_c" / "config_spot1.ini"},
    2: {"dir": MT5_DIR2, "ini": WINEPREFIX / "drive_c" / "config_spot2.ini"},
}
...
def read_accounts() -> dict[int, dict[str, str]]:
    """Parse acc.env -> {1: {'login':..,'password':..,'server':..}, 2: {...}}."""
    accounts: dict[int, dict[str, str]] = {1: {}, 2: {}}
    cur = 0
    try:
    ...
    for line in lines:
        ...
        if m:
            cur = int(m.group(1))
            line = m.group(2).strip()   # allow "account1 login = 123" one-liners
        if cur:
            for key in ("login", "password", "server"):
                m2 = re.match(rf"(?i)^{key}\s*=\s*(.+)$", line)
                if m2:
                    accounts[cur][key] = m2.group(1).strip()
    return accounts
...
def _exec_dir(inst: int) -> Path:
    ...
    Uses pick_terminal with the expected login from acc.env."""
...
def _write_start_cfg(inst: int) -> None:
    """Write the MT5 start config for a terminal instance.

    inst 1: only auto-attaches SpotDump (login comes from its stored accounts).
    inst 2: also auto-LOGINS account2 from acc.env ([Common] Login/Password/
    Server) because the copy has no stored account of its own.
    Symbol/Period guarantee a chart exists for the EA to attach to.
...
dies with retcode 10027 even when the algo button is on.
    AutoTrading=1 boots the terminal with the algo-trading button already ON
    so the executor never gets blocked by retcode 10027.
    """
    ini = TERMINALS[inst]["ini"]
    try:
        if inst == 2:
            a = read_accounts().get(2, {})
        else:
            a = read_accounts().get(1, {})
...
def launch_terminal(inst: int = 1) -> None:
