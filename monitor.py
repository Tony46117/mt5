...
something actually changes; noteworthy happenings (positions opened or
closed, balance or equity moves) land in a per-account EVENTS section on
the same screen - never a scrolling log.
...
class Monitor:
    ...
    def note(self, text: str) -> None:
        self.events.insert(0, (dt.datetime.now().strftime("%H:%M:%S"), text))
    ...
    def detect_changes(self, head: dict, trades: list[dict], mtime: float) -> bool:
        ...
        if self.prev_tickets is None:
            ...
            opened = tickets - self.prev_tickets
            closed = self.prev_tickets - tickets
            for t in trades:
                if t["ticket"] in opened:
                    self.note(f"OPENED  #{t['ticket'][-7:]} {t['symbol']} "
                              f"{t['side']} {t['volume']} @ {t['open']}")
            for tk in closed:
                self.note(f"CLOSED  #{tk[-7:]}")
            changed = changed or bool(opened or closed)
            self.prev_tickets = tickets
    ...
    def render(self, head: dict, trades: list[dict], mtime: float) -> str:
        ...
        if self.events:
            ...
            for ts, text in self.events:
                out.append(f"  {DIM}{ts}{RESET}  {text}")
            out.append("")
